...
// return : rien
// intérêt :  - fait avancer de 1 case le serpent vers le bas  
...
void move_down(std::vector<std::vector<char> > &gameboard,std::vector<std::vector<int> > &serpent,int &game_over)
{
    int speed = 1;
    ...
    new_coord.push_back(x_head);
    new_coord.push_back(y_head - 1);
    ...
    if (y_head - 1 <= 0 || y_head - 1 >= size_board -1)
    {
        game_over++;
    }
